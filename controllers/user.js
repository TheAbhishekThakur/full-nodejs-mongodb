...
// Like -> db.User.insertMany([{name:"journal"},{ name: "mat" }, { name: "mousepad"}])
// Insert multiple document

const insertManyDoc = async (req, res) => {
  try {
    let userArr = req.body.data;
    let created = await User.insertMany(userArr);
    if (created) {
      res.status(200).send(sendResponse(created, 0, "created"));
    }
  } catch (error) {
...
// Update one document
// Ex -> const res = await User.updateOne({ _id: "612096f93f688b126c0fd18a" }, { $set: { name: name });

const updateOneDocument = async (req, res) => {
  try {
    let name = req.body.name;
    let updated = await User.updateOne(
      { _id: "612096f93f688b126c0fd18a" },
      { $set: { name: name } }
    );
...
// Ex -> let updated = await User.updateMany({ isDeleted: isDeleted }, { $set: { name: name } });
...
// Get one document
// Ex -> let data = await User.findOne({ email: email });
