const QueAnsModel = require("../Models/QueAnsModel");
...
const createQueAns = async (req, res, next) => {
    try {
        let created = await QueAnsModel.create(req.body);
        if (created) {
...
const updateQueAns = async (req, res, next) => {
    try {
        ...
        let updated = await QueAnsModel.findOneAndUpdate({ _id: req.body._id }, { queAnsDup: updated_arr });
